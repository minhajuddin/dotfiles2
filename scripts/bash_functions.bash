recordandroid(){
  prefix=$@
  path="/sdcard/$prefix-recording-$(date +%Y%m%d%H%M%S).mp4"
  echo "Android video being recorded on the phone at $path"
...
log(){
  echo "$(date '+%Y-%m-%d %H:%M') $@" >> ~/Dropbox/log
}
